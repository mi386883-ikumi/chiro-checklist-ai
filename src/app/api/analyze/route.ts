...
ルール:
- checkItemsのcategoryは主訴に応じて動的に変える（4〜6カテゴリ）
- 特に「気になる症状の深掘り」「症状が出た時期」「その他のお悩み」「病歴・既往歴」を必ずカバーする
- **必ず最後に「推奨徒手検査」カテゴリを1つ追加すること**（後述の検査ルール参照）
...
「推奨徒手検査」カテゴリのルール:
- itemsには簡単に実施できる徒手検査を3〜5個提示する
- questionには検査名（例: 「SLRテスト」「ケンプテスト」）
- reasonには何を確認する検査か簡潔に（例: 「L5/S1神経根の圧迫所見を確認」）
- optionsは検査結果の選択肢として ["陽性", "陰性", "判定保留"] を基本とし、必要に応じて部位を追加（例: ["右陽性", "左陽性", "両側陽性", "陰性"]）
- 主訴に応じて適切な検査を選ぶ。例:
  * 腰痛: SLRテスト、ケンプテスト、ニュートンテスト、パトリックテスト、FNSテスト
  * 首こり・頚部痛: ジャクソンテスト、スパーリングテスト、頚椎可動域チェック
  * 肩関節痛: ペインフルアークサイン、ホーキンステスト、ニアテスト、エンプティカンテスト
  * 股関節痛: パトリック（FABER）テスト、トーマステスト、オーバーテスト
  * 膝痛: マクマレーテスト、ラックマンテスト、前後引き出しテスト
  * 上肢のしびれ: チンリフトテスト、ライトテスト、アドソンテスト
- 整体院で器具なしに行える徒手検査に限定する`
